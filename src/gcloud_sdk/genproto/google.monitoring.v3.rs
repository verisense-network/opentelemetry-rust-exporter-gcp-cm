...
pub mod alert_policy_service_server {
    ...
    impl<T: AlertPolicyService> tonic::server::NamedService
    for AlertPolicyServiceServer<T> {
...
pub mod metric_service_server {
    ...
    impl<T: MetricService> tonic::server::NamedService for MetricServiceServer<T> {
...
pub mod notification_channel_service_server {
    ...
    impl<T: NotificationChannelService> tonic::server::NamedService
    for NotificationChannelServiceServer<T> {
...
pub mod query_service_server {
    ...
    impl<T: QueryService> tonic::server::NamedService for QueryServiceServer<T> {
        const NAME: &'static str = "google.monitoring.v3.QueryService";
    }
}
...
pub mod service_monitoring_service_server {
    ...
    impl<T: ServiceMonitoringService> tonic::server::NamedService
    for ServiceMonitoringServiceServer<T> {
...
pub mod snooze_service_server {
    ...
    #[async_trait]
    pub trait SnoozeService: Send + Sync + 'static {
        /// Creates a `Snooze` that will prevent alerts, which match the provided
        /// criteria, from being opened. The `Snooze` applies for a specific time
        /// interval.
    ...
    /// or more alert policies should not fire alerts for the specified duration.
    #[derive(Debug)]
    pub struct SnoozeServiceServer<T: SnoozeService> {
        inner: Arc<T>,
        accept_compression_encodings: EnabledCompressionEncodings,
        send_compression_encodings: EnabledCompressionEncodings,
        max_decoding_message_size: Option<usize>,
    ...
    impl<T: SnoozeService> SnoozeServiceServer<T> {
        pub fn new(inner: T) -> Self {
            Self::from_arc(Arc::new(inner))
        }
        pub fn from_arc(inner: Arc<T>) -> Self {
    ...
    impl<T: SnoozeService> Clone for SnoozeServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: SnoozeService> tonic::server::NamedService for SnoozeServiceServer<T> {
        const NAME: &'static str = "google.monitoring.v3.SnoozeService";
    }
}
...
pub mod uptime_check_service_server {
    ...
    impl<T: UptimeCheckService> tonic::server::NamedService
    for UptimeCheckServiceServer<T> {
